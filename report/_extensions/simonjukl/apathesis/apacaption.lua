      ...
      if p.content[4] and p.content[4].text == ':' then
        ...
        for i, v in ipairs(p.content) do
          ...
          if i > intStart and i < intStart + 4 then
            if i == 3 then
              -- Figure or table number: use explicit labelnum if set, otherwise keep Quarto's
              if labelnum ~= "" then
                v = pandoc.Str(labelnum)
              end
              -- original: always overwrote with labelnum (breaks HTML book output where labelnum is always "")
              -- v = pandoc.Str(labelnum)
            end
