...
local getmode = function(meta)
  ...
  if meta["fig-pos"] then figpos = pandoc.utils.stringify(meta["fig-pos"]) end
  if meta["tbl-pos"] then tblpos = pandoc.utils.stringify(meta["tbl-pos"]) end
end
...
local processfloat = function(float)
  ...
  -- default float position from document metadata (fig-pos/tbl-pos)
  local defaultpos = float.type == "Table" and tblpos or figpos
  local floatposition = "[" .. defaultpos .. "]"
  local p = {}
  local apanotedivs = pandoc.Div(pandoc.Blocks {})
  if float.attributes["fig-pos"] and pandoc.utils.stringify(float.attributes["fig-pos"]) ~= "false" then
  ...
  if float.type == "Figure" then
    -- beforenote = "\\vspace{-4pt}\n" -- figures: less space than tables before note
    local hasnote = false
    local apanote
